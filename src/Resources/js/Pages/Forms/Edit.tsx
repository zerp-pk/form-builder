...
import { Plus, Trash2, ArrowUp, ArrowDown, Settings, Eye, Save, Type, Hash, Calendar, FileText, CheckSquare, List, Radio, Upload, Phone, Mail, Lock, Link, Clock, Zap } from 'lucide-react';
...
export default function EditForm({ form, auth }: EditFormProps) {
  ...
  const removeField = (id: string) => {
    ...
    // Check if field exists in database (has numeric ID and is not a temporary field)
    const isExistingField = field && !isNaN(Number(field.id)) && Number(field.id) > 0;
    
    if (isExistingField) {
      // For existing database fields, call API to delete
      ...
      // For new/temporary fields (string IDs or non-database fields), just remove from state
      setFields(fields.filter(field => field.id !== id));
    }
  };
